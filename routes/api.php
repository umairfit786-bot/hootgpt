...
Route::group(['prefix' => 'v2/external/chatbot', 'namespace' => 'App\Http\Controllers\Api\V2'], function() {
    Route::controller(ExternalChatbot::class)->group(function () {
        ...
        Route::post('/human-reply/{uuid}', 'humanReply');
        Route::get('/escalation-status/{uuid}/{conversationId}', 'escalationStatus');
    });
});
...
Route::group(['prefix' => 'v2/voice-agent'], function() {
    Route::controller(\App\Http\Controllers\Api\V2\VoiceAgentApiController::class)->group(function () {
        Route::get('/config/{uuid}', 'settings');
        Route::post('/signed-url/{uuid}', 'signedUrl');
        Route::post('/session/{uuid}', 'saveSession');
        Route::post('/query-knowledge/{uuid}', 'queryKnowledge');
    });
});