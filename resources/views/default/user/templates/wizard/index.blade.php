			...
			<div class="col-lg-4 col-md-12 col-sm-12">
										...
										<select id="language" name="language" class="form-select" data-placeholder="{{ __('Select input language') }}">		
											@foreach ($languages as $language)
												@if (!is_null($wizard['language']))
													<option value="{{ $language->language_code }}" data-img="{{ theme_url($language->language_flag) }}" @if ($wizard['language'] == $language->language) selected @endif> {{ __($language->language) }}</option>
												@else
				...
				<div id="final-wizard-status" class="hidden">
					<div class="row justify-content-center mt-6">
						<div class="col-sm-12 text-center">
							<i class="fa-solid   fa-sparkles mb-7 fs-24 text-primary"></i>
			...
			$.ajax({
				...
				success: function (data) {	
					...
					if (data['status'] == 'error') {
						...
						const eventSource = new EventSource( "/app/user/wizard/generate/process?wizard=" + data.wizard_id+"&content=" + data.content_id);
