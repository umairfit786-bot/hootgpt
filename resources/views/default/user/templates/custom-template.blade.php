...
<form id="openai-form" action="" method="post" enctype="multipart/form-data" class="mt-24"> 		
	@csrf
	<div class="row">	
		<div class="col-xl-4 col-lg-6 col-md-12 col-sm-12 border-template">
								...
								<div>
									<h6 class="mt-1 ml-3 fs-16 number-font">{{ __($template->name) }} 										
										@if($template->package == 'professional') 
											<span class="fs-8 btn btn-pro ml-2"><i class="fa-solid fa-crown mr-2"></i>{{ __('Pro') }} </span> 
										@elseif($template->package == 'free')
											<span class="fs-8 btn btn-free ml-2"><i class="fa-solid fa-gift mr-2"></i>{{ __('Free') }} </span> 
										@elseif($template->package == 'premium')
											<span class="fs-8 btn btn-yellow ml-2"><i class="fa-solid fa-gem mr-2"></i>{{ __('Premium') }} </span> 
										@elseif($template->new)
											<span class="fs-8 btn btn-new ml-2"><i class="fa-solid fa-sparkles mr-2"></i>{{ __('New') }}</span>
										@endif	
									</h6>
								</div>
								<div>
								...
								@if (App\Services\HelperService::checkWordpressIntegrationFeature())
									<div class="col-lg-2 col-md-6 col-sm-12">
										<div class="btn-group mt-1 publish-action-buttons">
											...
											<div class="dropdown-menu" aria-labelledby="export" data-popper-placement="bottom-start">								
												<a class="dropdown-item" id="publish" href="#"><i class="fa-brands fa-wordpress fs-13 text-muted mr-2"></i> {{ __('Publish to Wordpress') }}</a>
											</div>
										</div>
									</div>
								@endif
...
<script type="text/javascript">
	...
	$(function () {
		...
		$('#publish').on('click', function(e) {
			e.preventDefault();
			let saveButton = document.getElementById('save-button-template');
			if (!saveButton.hasAttribute('target')) {
				toastr.warning('{{ __("Make sure to save your generated content first") }}');
				return;
			} else {
				let id = saveButton.getAttribute('target');

				@if (App\Services\HelperService::checkIntegrationAccess())
					@if (App\Services\HelperService::extensionWordpressIntegration())
						@if (App\Services\HelperService::checkWordpressIntegrationFeature())
						let url = "{{ route('user.integration.wordpress.post.create', ['id' => ':id']) }}";
						@endif
					@endif
				@endif
		...
		$('#openai-form').on('submit', function(e) {
			...
			$.ajax({
				headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
				method: 'POST',
				url: '/app/user/templates/custom-template/customGenerate',
				data: form.serialize(),
				beforeSend: function() {
				...
				success: function (data) {	
					...
					if (data['status'] == 'error') {
						...
						const eventSource = new EventSource( "/app/user/templates/original-template/process?content_id=" + data.id+"&max_results=" + data.max_results + "&max_words=" + data.max_words + "&temperature=" + data.temperature);
