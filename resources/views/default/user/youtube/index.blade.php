							...
							<div class="form-group">	
								...
								<select id="language" name="language" class="form-select" data-placeholder="{{ __('Select input language') }}">		
									@foreach ($languages as $language)
										<option value="{{ $language->language_code }}" data-img="{{ theme_url($language->language_flag) }}" @if (auth()->user()->default_template_language == $language->language_code) selected @endif> {{ $language->language }}</option>
									@endforeach									
								</select>
								@error('language')
			...
			$.ajax({
				...
				success: function (data) {	
					...
					if (data['status'] == 'error') {
						...
						const eventSource = new EventSource( "/app/user/youtube/process?content_id=" + data.id+"&model=" + data.model + "&temperature=" + data.temperature + "&language=" + data.language);
