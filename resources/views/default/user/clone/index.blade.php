...
@section('content')
	<div class="row mt-24">
			...
			<div class="card border-0">
				<div class="card-header pt-4 border-0" id="voiceover-character-counter-top">
					<h3 class="card-title"><i
							class="   fa-solid fa-user-music mr-4 text-info"></i>{{ __('Instant Voice Cloning') }} </h3>
					<span class="fs-11 text-muted pl-3"><i
							class="fa-solid fa-microphone mr-2 mt-1 text-primary"></i>{{ __('Clones Allowed') }}: <span
							class="font-weight-semibold">{{ $voices_count }} /
							{{ $voice_limit == -1 ? __('Unlimited') : $voice_limit }}</span></span>
				</div>
				<form id="create-voice-form" action="{{ route('user.voiceover.clone.create') }}" method="POST"
					enctype="multipart/form-data">
					@csrf
					<div class="card-body pt-2 pl-6 pr-6 pb-6" id="">

						<div class="input-box" style="position: relative">
							<h6>{{ __('New Voice Name') }} <span class="text-required"><i
						...
						<div class="row">
							<div class="col-lg-10 col-md-10 col-sm-10">
								<div class="input-box">
									<h6>{{ __('Re-Train Existing Voice') }} <i
											class="ml-2 text-dark fs-13 fa-solid fa-circle-info"
											data-tippy-content="{{ __('For re-training your existing custom voice clone, select the target voice from the list and upload additional audio samples to further train the selected voice and click the Train Voice button.') }}."></i>
									</h6>
							...
							<div class="col-lg-2 col-md-2 col-sm-2">
								<div class="dropdown w-100 mt-4">
									<button class="btn btn-special create-project" type="button" id="delete-voice"
										data-tippy-content="{{ __('Delete Voice Clone') }}"><i
											class="fa-solid fa-music-note-slash"></i></button>
								</div>
							</div>
						</div>

						<div class="input-box" style="position: relative">
							<h6 class="mb-0">{{ __('Audio Samples') }} <span class="text-required"><i
										class="fa-solid fa-asterisk"></i></span></h6>
							<div id="image-drop-box">
								<div class="image-drop-area text-center mt-2 file-drop-border">
									<input type="file" class="main-image-input" name="file[]" id="file" multiple
										accept=".mp3" required>
						...
						<div class="input-box">
							...
							<textarea class="form-control" name="description" rows="4" id="description"
								placeholder="{{ __('How would you describe the voice?') }} e.g. &quot;{{ __('An old British male voice with a slight hoarseness in his throat. Perfect for news.') }}&quot;"></textarea>
						</div>

						<div class="text-center">
							<button type="button" class="btn btn-primary ripple main-action-button" id="create-voice"
								style="text-transform: none; min-width: 200px;">{{ __('Create Voice') }}</button>
							<button type="button" class="btn btn-primary ripple main-action-button" id="train-voice"
								style="text-transform: none; min-width: 200px;">{{ __('Re-Train Voice') }}</button>
						</div>
					</div>
				</form>
			</div>
		...
		<div class="col-lg-8 col-md-12 col-xm-12">
			<div class="card border-0">
				<div class="card-header pt-4 border-0" id="voiceover-character-counter-top">
					<h3 class="card-title"><i
							class="   fa-solid fa-waveform-lines mr-4 text-info"></i>{{ __('AI Text to Speech') }} </h3>
					<span class="fs-11 text-muted pl-3" id="voiceover-character-counter"><i
							class="   fa-solid fa-bolt-lightning mr-2 text-primary"></i>{{ __('Your Balance is') }} <span
							class="font-weight-semibold" id="balance-number">@if (auth()->user()->characters == -1)
							{{ __('Unlimited') }} @else
								{{ number_format(auth()->user()->characters + auth()->user()->characters_prepaid) }}
							{{ __('Characters') }} @endif</span></span>
				</div>
				<div class="card-body pt-2 pl-6 pr-6 pb-4" id="tts-body-minify">

					<form id="synthesize-text-form" action="{{ route('user.voiceover.clone.synthesize') }}"
						listen="{{ route('user.voiceover.clone.listen') }}" method="POST" enctype="multipart/form-data">
						@csrf
						...
						<div class="row mb-4" id="tts-awselect">
							...
							<div class="col-lg-6 col-md-12 col-sm-12 mb-4">
								<div class="form-group">
									<select id="project" name="project" class="form-select"
										data-placeholder="{{ __('Select Workbook Name') }}">
										<option value="all"> {{ __('All Workbooks') }}</option>
										@foreach ($projects as $project)
											<option value="{{ $project->name }}" @if (strtolower(auth()->user()->project) == strtolower($project->name)) selected
											@endif> {{ ucfirst($project->name) }}</option>
										@endforeach
									</select>
								</div>
							</div>
							...
							<div class="col-lg-6 col-md-12 col-sm-12">
								<div class="form-group">
									<select id="languages" name="language" class="form-select"
										...
										<option value="ar-AE" data-img="{{ theme_url('img/flags/ae.svg') }}" @if (auth()->user()->default_voiceover_language == 'ar-AE') selected @endif>
											{{ __('Arabic') }}</option>
										<option value="bg-BG" data-img="{{ theme_url('img/flags/bg.svg') }}" @if (auth()->user()->default_voiceover_language == 'bg-BG') selected @endif>
											{{ __('Bulgarian (Bulgaria)') }}</option>
										<option value="cmn-CN" data-img="{{ theme_url('img/flags/cn.svg') }}" @if (auth()->user()->default_voiceover_language == 'cmn-CN') selected @endif>
											{{ __('Chinese (Mandarin)') }}</option>
										<option value="hr-HR" data-img="{{ theme_url('img/flags/hr.svg') }}" @if (auth()->user()->default_voiceover_language == 'hr-HR') selected @endif>
											{{ __('Croatian (Croatia)') }}</option>
										<option value="cs-CZ" data-img="{{ theme_url('img/flags/cz.svg') }}" @if (auth()->user()->default_voiceover_language == 'cs-CZ') selected @endif>
											{{ __('Czech (Czech Republic)') }}</option>
										<option value="da-DK" data-img="{{ theme_url('img/flags/dk.svg') }}" @if (auth()->user()->default_voiceover_language == 'da-DK') selected @endif>
											{{ __('Danish (Denmark)') }}</option>
										<option value="nl-NL" data-img="{{ theme_url('img/flags/nl.svg') }}" @if (auth()->user()->default_voiceover_language == 'nl-NL') selected @endif>
											{{ __('Dutch (Netherlands)') }}</option>
										<option value="en-US" data-img="{{ theme_url('img/flags/us.svg') }}" @if (auth()->user()->default_voiceover_language == 'en-US') selected @endif>
											{{ __('English (USA)') }}</option>
										<option value="fil-PH" data-img="{{ theme_url('img/flags/ph.svg') }}" @if (auth()->user()->default_voiceover_language == 'fil-PH') selected @endif>
											{{ __('Filipino (Philippines)') }}</option>
										<option value="fi-FI" data-img="{{ theme_url('img/flags/fi.svg') }}" @if (auth()->user()->default_voiceover_language == 'fi-FI') selected @endif>
											{{ __('Finnish (Finland)') }}</option>
										<option value="fr-FR" data-img="{{ theme_url('img/flags/fr.svg') }}" @if (auth()->user()->default_voiceover_language == 'fr-FR') selected @endif>
											{{ __('French (France)') }}</option>
										<option value="de-DE" data-img="{{ theme_url('img/flags/de.svg') }}" @if (auth()->user()->default_voiceover_language == 'de-DE') selected @endif>
											{{ __('German (Germany)') }}</option>
										<option value="el-GR" data-img="{{ theme_url('img/flags/gr.svg') }}" @if (auth()->user()->default_voiceover_language == 'el-GR') selected @endif>
											{{ __('Greek (Greece)') }}</option>
										<option value="hi-IN" data-img="{{ theme_url('img/flags/in.svg') }}" @if (auth()->user()->default_voiceover_language == 'hi-IN') selected @endif>
											{{ __('Hindi (India)') }}</option>
										<option value="id-ID" data-img="{{ theme_url('img/flags/id.svg') }}" @if (auth()->user()->default_voiceover_language == 'id-ID') selected @endif>
											{{ __('Indonesian (Indonesia)') }}</option>
										<option value="it-IT" data-img="{{ theme_url('img/flags/it.svg') }}" @if (auth()->user()->default_voiceover_language == 'it-IT') selected @endif>
											{{ __('Italian (Italy)') }}</option>
										<option value="ja-JP" data-img="{{ theme_url('img/flags/jp.svg') }}" @if (auth()->user()->default_voiceover_language == 'ja-JP') selected @endif>
											{{ __('Japanese (Japan)') }}</option>
										<option value="ko-KR" data-img="{{ theme_url('img/flags/kr.svg') }}" @if (auth()->user()->default_voiceover_language == 'ko-KR') selected @endif>
											{{ __('Korean (South Korea)') }}</option>
										<option value="ms-MY" data-img="{{ theme_url('img/flags/my.svg') }}" @if (auth()->user()->default_voiceover_language == 'ms-MY') selected @endif>
											{{ __('Malay (Malaysia)') }}</option>
										<option value="pl-PL" data-img="{{ theme_url('img/flags/pl.svg') }}" @if (auth()->user()->default_voiceover_language == 'pl-PL') selected @endif>
											{{ __('Polish (Poland)') }}</option>
										<option value="pt-PT" data-img="{{ theme_url('img/flags/pt.svg') }}" @if (auth()->user()->default_voiceover_language == 'pt-PT') selected @endif>
											{{ __('Portuguese (Portugal)') }}</option>
										<option value="ro-RO" data-img="{{ theme_url('img/flags/ro.svg') }}" @if (auth()->user()->default_voiceover_language == 'ro-RO') selected @endif>
											{{ __('Romanian (Romania)') }}</option>
										<option value="ru-RU" data-img="{{ theme_url('img/flags/ru.svg') }}" @if (auth()->user()->default_voiceover_language == 'ru-RU') selected @endif>
											{{ __('Russian (Russia)') }}</option>
										<option value="sk-SK" data-img="{{ theme_url('img/flags/sk.svg') }}" @if (auth()->user()->default_voiceover_language == 'sk-SK') selected @endif>
											{{ __('Slovak (Slovakia)') }}</option>
										<option value="es-ES" data-img="{{ theme_url('img/flags/es.svg') }}" @if (auth()->user()->default_voiceover_language == 'es-ES') selected @endif>
											{{ __('Spanish (Spain)') }}</option>
										<option value="sv-SE" data-img="{{ theme_url('img/flags/se.svg') }}" @if (auth()->user()->default_voiceover_language == 'sv-SE') selected @endif>
											{{ __('Swedish (Sweden)') }}</option>
										<option value="ta-MY" data-img="{{ theme_url('img/flags/my.svg') }}" @if (auth()->user()->default_voiceover_language == 'ta-MY') selected @endif>
											{{ __('Tamil (Malaysia)') }}</option>
										<option value="tr-TR" data-img="{{ theme_url('img/flags/tr.svg') }}" @if (auth()->user()->default_voiceover_language == 'tr-TR') selected @endif>
											{{ __('Turkish (Turkey)') }}</option>
										<option value="uk-UA" data-img="{{ theme_url('img/flags/ua.svg') }}" @if (auth()->user()->default_voiceover_language == 'uk-UA') selected @endif>
							...
							<div class="col-lg-6 col-md-12 col-sm-12">
								<div class="form-group">
									<select id="voices" name="voice" class="form-select"
										data-placeholder="{{ __('Choose Your Voice') }}:" data-callback="voice_select">
										@foreach ($voices as $voice)
											<option value="{{ $voice->voice_id }}" id="{{ $voice->voice_id }}"
												data-img="@if ($voice->vendor == 'elevenlabs') {{ $voice->avatar_url }} @else {{ theme_url($voice->avatar_url) }} @endif"
												data-id="{{ $voice->voice_id }}" data-type="{{ __('Custom Voice') }}"
								...
								<div id="textarea-outer-box" class="p-2">
									...
									<div id="textarea-container">
										<div id="textarea-row-box">
											<div class="textarea-row pl-2 pr-0" id="maintextarea">
												<div class="textarea-voice pl-0 mr-2">
													<div class="ml-1 mt-1 voicee"><img
															src="{{ theme_url('img/files/avatar.webp') }}" id="ZZZOOOVVVIMG"
															data-tippy-content=""></div>
												</div>
												<div class="textarea-text ml-0 mr-3">
													<textarea class="form-control textarea" name="textarea[]"
														id="ZZZOOOVVVZ" data-voice="" onkeyup="countCharacters();"
														onmousedown="mouseDown(this);" rows="1"
														placeholder="{{ __('Enter your text here to synthesize') }}..."
														maxlength="5000"></textarea>
												</div>
												<div class="textarea-actions">
													<div class="textarea-buttons">
														<button class="btn buttons addPause mr-0" id="ZZZOOOVVVP"
															onclick="addPause(this); return false;"
															data-tippy-content="{{ __('Add Pause After Text') }}"><i
																class="fa-regular fa-hourglass-clock"></i></button>
														<button type="button" class="btn buttons deleteText mr-0"
															id="ZZZOOOVVVDEL" onclick="deleteRow(this); return false;"
															data-tippy-content="{{ __('Delete This Text Block') }}"><i
										...
										<div class="textarea-add text-center mt-2">
											<button class="btn" id="addTextRow"
												data-tippy-content="{{ __('Add New Text Block') }}"><i
													class="fa-solid fa-layer-plus"></i></button>
										</div>
									</div>
									<div id="textarea-settings">
										...
										<div class="clear-button">
											<button type="button"
												id="delete-all-lines">{{ __('Delete All Lines') }}</button>
							...
							<div class="row">
								...
								<div class="col-sm-12">
									<div id="controls" class="mt-4 mb-3">
										...
										<a id="downloadBtn" class="result-play result-play-sm" href="" download><i
						...
						<div class="card-footer border-0 text-center mt-3">
							<button type="button" class="btn btn-primary ripple main-action-button mr-2" id="listen-text"
								style="text-transform: none; min-width: 160px;">{{ __('Listen') }}</button>
							<button type="button" class="btn btn-primary ripple main-action-button" id="synthesize-text"
								style="text-transform: none; min-width: 160px;">{{ __('Synthesize') }}</button>
						</div>
	...
	<!-- DELETE VOICE MODAL -->
	<div class="modal fade" id="deleteVoiceModal" tabindex="-1" role="dialog" aria-labelledby="projectModalLabel"
		aria-hidden="true" data-bs-keyboard="false">
		<div class="modal-dialog modal-dialog-centered modal-md" role="document">
			<div class="modal-content">
				<div class="modal-header mb-1">
					<h4 class="modal-title" id="myModalLabel"><i class="fa-solid fa-music-note-slash"></i>
						{{ __('Delete Voice Clone') }}</h4>
				...
				<div class="modal-body pb-0 pl-6 pr-6">
					<p class="text-danger mb-3 fs-12">
						{{ __('Warning! This will permanently delete this custom voice clone') }}</p>
					<div class="input-box">
...
@section('js')
	...
	<script src="{{theme_url('js/dashboard-clone.js')}}"></script>
	<script type="text/javascript">
		...
		$(function () {
			...
			var table = $('#resultTable').DataTable({
				...
				colReorder: true,
				language: {
					"emptyTable": "<div><img id='no-results-img' src='{{ theme_url('img/files/no-result.png') }}'><br>{{ __('No synthesized text results yet') }}</div>",
				...
				Swal.fire({
					...
					if (result.isConfirmed) {
						...
						$.ajax({
							...
							method: 'post',
							url: '/app/user/text-to-speech/clone/delete',
							data: formData,
							processData: false,
							contentType: false,
			...
			$('#del-voice-button').on('click', function (e) {
				...
				if (voice == 'none') {
					...
					$.ajax({
						...
						type: "POST",
						url: '/app/user/text-to-speech/clone/voice/remove',
						data: { 'id': voice },
						success: function (data) {
							if (data == 'success') {
