			...
			<div class="col-lg-4 col-md-12 col-sm-12">
				<div class="card border-0">
					<div class="card-header pt-4 border-0" id="voiceover-character-counter-top">
						<h3 class="card-title"><i class=" fa-solid fa-user-music mr-4 text-info"></i>{{ __('Speechify Voice Cloning') }} </h3>
					</div>
					<form id="create-voice-form" action="{{ route('user.speechify.clone.create') }}" method="POST" enctype="multipart/form-data">
						@csrf
						<div class="card-body pt-2 pl-0 pr-0 pb-6" id="">
							...
							<div class="input-box" style="position: relative">
								...
								<div id="image-drop-box">
									<div class="image-drop-area text-center mt-2 file-drop-border pl-4 pr-4">
										<input type="file" class="main-image-input" id="file" multiple accept=".mp3" required>
							...
							<div class="mb-4">
								<h6 class="font-weight-bold fs-11 mb-3">{{ __('User Consent') }}</h6>
								
								<div class="input-box" style="position: relative">	
									<h6 class="text-muted">{{ __('Full Name') }} <span class="text-required"><i class="fa-solid fa-asterisk"></i></span></h6>
									<input type="text" class="form-control @error('fullName') is-danger @enderror" id="fullName" name="fullName" value="{{ old('fullName') }}" required>
								</div>

								<div class="input-box" style="position: relative">	
									<h6 class="text-muted">{{ __('Email') }} <span class="text-required"><i class="fa-solid fa-asterisk"></i></span></h6>
									<input type="email" class="form-control @error('email') is-danger @enderror" id="email" name="email" value="{{ old('email') }}" required>
								</div>
							</div>
							...
							<div class="text-center">
								<button type="button" class="btn btn-primary ripple main-action-button mb-4" id="create-voice" >{{ __('Create Voice') }}</button>
							</div>
						</div>
					</form>
				</div>
			...
			<div class="col-lg-8 col-md-12 col-xm-12">
				<div class="card border-0">
					...
					<div class="card-body pt-2 pl-6 pr-6 pb-4" id="tts-body-minify">
							...
							<form id="synthesize-text-form" action="{{ route('user.speechify.synthesize') }}" listen="{{ route('user.speechify.listen') }}" method="POST" enctype="multipart/form-data">
								@csrf							
									...
									<div class="col-lg-6 col-md-12 col-sm-12">
										<div class="form-group">									
											<select id="languages" name="language" class="form-select" data-placeholder="{{ __('Pick Your Language') }}:">	
												<option value="ar-AE" data-img="{{ theme_url('img/flags/ae.svg') }}" @if (auth()->user()->default_voiceover_language == 'ar-AE') selected @endif> {{ __('Arabic') }}</option>																				
												<option value="da-DK" data-img="{{ theme_url('img/flags/dk.svg') }}" @if (auth()->user()->default_voiceover_language == 'da-DK') selected @endif> {{ __('Danish (Denmark)') }}</option>										
												<option value="nl-NL" data-img="{{ theme_url('img/flags/nl.svg') }}" @if (auth()->user()->default_voiceover_language == 'nl-NL') selected @endif> {{ __('Dutch (Netherlands)') }}</option>										
												<option value="en" data-img="{{ theme_url('img/flags/us.svg') }}" @if (auth()->user()->default_voiceover_language == 'en-US') selected @endif> {{ __('English') }}</option>																			
												<option value="et-EE" data-img="{{ theme_url('img/flags/ee.svg') }}" @if (auth()->user()->default_voiceover_language == 'et-EE') selected @endif> {{ __('Estonian (Estonia)') }}</option>										
												<option value="fi-FI" data-img="{{ theme_url('img/flags/fi.svg') }}" @if (auth()->user()->default_voiceover_language == 'fi-FI') selected @endif> {{ __('Finnish (Finland)') }}</option>										
												<option value="fr-FR" data-img="{{ theme_url('img/flags/fr.svg') }}" @if (auth()->user()->default_voiceover_language == 'fr-FR') selected @endif> {{ __('French') }}</option>										
												<option value="de-DE" data-img="{{ theme_url('img/flags/de.svg') }}" @if (auth()->user()->default_voiceover_language == 'de-DE') selected @endif> {{ __('German (Germany)') }}</option>										
												<option value="el-GR" data-img="{{ theme_url('img/flags/gr.svg') }}" @if (auth()->user()->default_voiceover_language == 'el-GR') selected @endif> {{ __('Greek (Greece)') }}</option>										
												<option value="hi-IN" data-img="{{ theme_url('img/flags/in.svg') }}" @if (auth()->user()->default_voiceover_language == 'hi-IN') selected @endif> {{ __('Hindi (India)') }}</option>																			
												<option value="it-IT" data-img="{{ theme_url('img/flags/it.svg') }}" @if (auth()->user()->default_voiceover_language == 'it-IT') selected @endif> {{ __('Italian (Italy)') }}</option>										
												<option value="ja-JP" data-img="{{ theme_url('img/flags/jp.svg') }}" @if (auth()->user()->default_voiceover_language == 'ja-JP') selected @endif> {{ __('Japanese (Japan)') }}</option>																		
												<option value="pl-PL" data-img="{{ theme_url('img/flags/pl.svg') }}" @if (auth()->user()->default_voiceover_language == 'pl-PL') selected @endif> {{ __('Polish (Poland)') }}</option>										
												<option value="pt-PT" data-img="{{ theme_url('img/flags/pt.svg') }}" @if (auth()->user()->default_voiceover_language == 'pt-PT') selected @endif> {{ __('Portuguese (Portugal)') }}</option>										
												<option value="ro-RO" data-img="{{ theme_url('img/flags/ro.svg') }}" @if (auth()->user()->default_voiceover_language == 'ro-RO') selected @endif> {{ __('Romanian (Romania)') }}</option>										
												<option value="ru-RU" data-img="{{ theme_url('img/flags/ru.svg') }}" @if (auth()->user()->default_voiceover_language == 'ru-RU') selected @endif> {{ __('Russian (Russia)') }}</option>																			
												<option value="es-ES" data-img="{{ theme_url('img/flags/es.svg') }}" @if (auth()->user()->default_voiceover_language == 'es-ES') selected @endif> {{ __('Spanish (Spain)') }}</option>										
												<option value="sv-SE" data-img="{{ theme_url('img/flags/se.svg') }}" @if (auth()->user()->default_voiceover_language == 'sv-SE') selected @endif> {{ __('Swedish (Sweden)') }}</option>																		
												<option value="tr-TR" data-img="{{ theme_url('img/flags/tr.svg') }}" @if (auth()->user()->default_voiceover_language == 'tr-TR') selected @endif> {{ __('Turkish (Turkey)') }}</option>										
												<option value="uk-UA" data-img="{{ theme_url('img/flags/ua.svg') }}" @if (auth()->user()->default_voiceover_language == 'uk-UA') selected @endif> {{ __('Ukrainian (Ukraine)') }}</option>																	
											</select>
										</div>
									</div>
												...
												<div id="textarea-row-box">
													<div class="textarea-row pl-2 pr-0" id="maintextarea">
														<div class="textarea-voice pl-0 mr-2">
															<div class="ml-1 mt-1 voicee"><img src="{{ theme_url('img/files/avatar.webp') }}" id="ZZZOOOVVVIMG"  data-tippy-content=""></div>
														</div>
	...
	<script type="text/javascript">
		...
		$(function () {
			...
			var table = $('#resultTable').DataTable({
				...
				processing: true,
				serverSide: true,
				ajax: "{{ route('user.speechify.clone') }}",
				columns: [{
				...
				Swal.fire({
					...
					if (result.isConfirmed) {
						...
						$.ajax({
							...
							method: 'post',
							url: '/app/user/voice-clone/delete',
							data: formData,
							processData: false,
							contentType: false,
			...
			$('#create-voice').on('click',function(e) {
				...
				$.ajax({
					...
					type: "POST",
					url: '{{ route('user.speechify.clone.create') }}',
					data: data,
					processData: false,
					contentType: false,
					beforeSend: function() {
					...
					error: function(data) {
						let errorMsg = data.responseJSON?.message || data.responseJSON?.error || '{{ __('An error occurred') }}';
