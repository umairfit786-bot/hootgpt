						...
						<div class="col-lg-10 col-md-9 col-sm-12">
							<div class="row">
								<div class="col-lg-5 col-sm-12">
									<h6 class="fs-12 plan-title-bar"><span class="font-weight-bold">{{__('Step 1')}}</span> : {{__('Select your Avatar Image')}}</h6>
									<div class="avatar-images-border-line">
										<div class="row p-4">
											@foreach ($favorites as $favorite)
												@foreach ($avatars['data']['talking_photos'] as $avatar)
														...
														<div class="col-md-6 col-sm-12">
															<div class="card avatar-card mb-6" data-id="{{ $avatar['talking_photo_id'] }}">
																<div class="avatar-card-box">
																	...
																	<div class="avatar-image-box talking-photos-generate">
								...
								<div class="col-lg-7 col-sm-12">
									<form id="generate-avatar-form" action="{{ route('user.extension.avatar.image.create.store') }}" method="POST" enctype="multipart/form-data">
										@csrf
										<div class="ml-5 mr-0 mb-5 mt-0">
											...
											<div class="avatar-input-text">
												...
												<div id="form-group" class="mb-5">
													<h6 class="fs-11 mb-2 font-weight-semibold">{{ __('Avatar Crop Style') }}</h6>
													<select id="talking_photo_style" name="talking_photo_style" class="form-select">													
												...
												<div id="form-group" class="mb-5">
													<h6 class="fs-11 mb-2 font-weight-semibold">{{ __('Avatar Talking Style') }}</h6>
													<select id="talking_style" name="talking_style" class="form-select">													
														<option value="stable">{{__('Stable')}}</option>			
														<option value="expressive">{{__('Expressive')}}</option>			
													</select>
												</div>

												<div id="form-group" class="mb-5">
													<h6 class="fs-11 mb-2 font-weight-semibold">{{ __('Avatar Expression') }}</h6>
													<select id="expression" name="expression" class="form-select">													
				...
				if (avatar_id == '') {
					...
					$.ajax({
						...
						success: function(data) {
							...
							if (data == 502) {
								toastr.success('{{ __('Not enough credits to generate Avatar from Image, please upgrade') }}');
							}
