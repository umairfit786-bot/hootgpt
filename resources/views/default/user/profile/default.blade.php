			...
			<form method="POST" class="w-100" action="{{ route('user.profile.update.defaults') }}" enctype="multipart/form-data">
				...
				<div class="card  ">
					...
					<div class="card-body pb-0">					
									...
									<select id="model" name="default_model_template" class="form-select" >		
										@foreach ($models as $model)
											...
											@elseif (trim($model) == 'gpt-5')
												<option value="{{ trim($model) }}" @if (trim($model) == auth()->user()->default_model_template) selected @endif>{{ __('GPT 5') }}</option>
											@elseif (trim($model) == 'gpt-5-mini')
												<option value="{{ trim($model) }}" @if (trim($model) == auth()->user()->default_model_template) selected @endif>{{ __('GPT 5 mini') }}</option>
											@elseif (trim($model) == 'gpt-5-nano')
												<option value="{{ trim($model) }}" @if (trim($model) == auth()->user()->default_model_template) selected @endif>{{ __('GPT 5 nano') }}</option>
											@elseif (trim($model) == 'gpt-5-chat-latest')
												<option value="{{ trim($model) }}" @if (trim($model) == auth()->user()->default_model_template) selected @endif>{{ __('GPT 5 chat') }}</option>
								...
								<div class="form-group">	
									...
									<select id="model" name="default_model_chat" class="form-select" >		
										@foreach ($models as $model)
											...
											@elseif (trim($model) == 'gpt-5')
												<option value="{{ trim($model) }}" @if (trim($model) == auth()->user()->default_model_chat) selected @endif>{{ __('GPT 5') }}</option>
											@elseif (trim($model) == 'gpt-5-mini')
												<option value="{{ trim($model) }}" @if (trim($model) == auth()->user()->default_model_chat) selected @endif>{{ __('GPT 5 mini') }}</option>
											@elseif (trim($model) == 'gpt-5-nano')
												<option value="{{ trim($model) }}" @if (trim($model) == auth()->user()->default_model_chat) selected @endif>{{ __('GPT 5 nano') }}</option>
											@elseif (trim($model) == 'gpt-5-chat-latest')
												<option value="{{ trim($model) }}" @if (trim($model) == auth()->user()->default_model_chat) selected @endif>{{ __('GPT 5 chat') }}</option>
											@elseif (trim($model) == 'claude-3-opus-20240229')
										...
										@if (App\Services\HelperService::extensionNanoBanana())
											@foreach ($vendors as $vendor)
												@if (trim($vendor) == 'google')
													<option value="gemini-2.5-flash-image" @if (auth()->user()->default_image_model == 'gemini-2.5-flash-image') selected @endif>{{ __('Nano Banana') }}</option>
													<option value="gemini-3-pro-image-preview" @if (auth()->user()->default_image_model == 'gemini-3-pro-image-preview') selected @endif>{{ __('Nano Banana Pro') }}</option>
												@endif
											@endforeach
										@endif
									</select>	
						...
						<div class="card-footer border-0  text-center mt-2 mb-2 pr-0">
