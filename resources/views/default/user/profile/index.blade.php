					...
					<div class="card mb-5  ">
						<div class="card-header d-inline  ">
							...
							@else
								...
								<div class="mb-1">
									<span class="fs-12 text-muted">{{ $user_subscription->plan_name ?? '' }} {{ __('Plan') }} / {!! config('payment.default_system_currency_symbol') !!}{{ $user_subscription->price ?? '0.00' }} @if ($user_subscription->payment_frequency == 'monthly') {{ __('Per Month') }} @elseif($user_subscription->payment_frequency == 'yearly') {{ __('Per Year') }} @else {{ __('One Time Payment') }} @endif</span>
								</div>
							@endif
						</div>
						<div class="card-body">
