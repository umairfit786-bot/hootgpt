...
class ImageController extends Controller
{
    ...
    {   
        ...
        switch (auth()->user()->default_image_model) {
            ...
            case 'gpt-image-1': $model = 'gpt-image-1'; $model_name = 'GPT Image'; break;
            case 'gpt-image-1-mini': $model = 'gpt-image-1-mini'; $model_name = 'GPT Image Mini'; break;
            case 'gpt-image-1.5': $model = 'gpt-image-1.5'; $model_name = 'GPT Image 1.5'; break;
            ...
            case 'sd3.5-flash': $model = 'sd3.5-flash'; $model_name = 'SD 3.5 Flash'; break; 		
            ...
            case 'gemini-2.5-flash-image': $model = 'gemini-2.5-flash-image'; $model_name = 'Nano Banana'; break;
            case 'gemini-3-pro-image-preview': $model = 'gemini-3-pro-image-preview'; $model_name = 'Nano Banana Pro'; break;
        ...
        if ($request->ajax()) {
            ...
            if ($request->vendor == 'openai') {
                ...
                for ($i=0; $i < $multi_image; $i++) { 
                    ...
                    } elseif ($request->model == 'gpt-image-1' || $request->model == 'gpt-image-1-mini' || $request->model == 'gpt-image-1.5') {
                        $complete = $open_ai->image([
                            'model' => $request->model,
                            'prompt' => $prompt,
                            'size' => $request->resolution,
                    ...
                    if (isset($response['data'])) {
                        if (count($response['data']) > 1) {
                            foreach ($response['data'] as $key => $value) {
                                ...
                                if ($request->model == 'gpt-image-1' || $request->model == 'gpt-image-1-mini' || $request->model == 'gpt-image-1.5') {
                                    $contents = base64_decode($value['b64_json']);
                                } else {
                                    $url = $value['url'];
                ...
                if ($sd_model != 'core' && $sd_model != 'ultra' && $sd_model != 'sd3.5-large' && $sd_model != 'sd3.5-large-turbo' && $sd_model != 'sd3.5-medium' && $sd_model != 'sd3.5-flash') {
                    ...
                    for ($i=0; $i < $max_results; $i++) { 
                        ...
                        if ($sd_model == 'sd3.5-flash' || $sd_model == 'sd3.5-medium' || $sd_model == 'sd3.5-large' || $sd_model == 'sd3.5-large-turbo') {
                ...
                for ($i=0; $i < $max_results; $i++) {
                    ...
                    if ($response['status'] == 'success') {                    
                        ...
                        do {
                            $result = $midjourney->status($response['result']);
                            Log::info('in do while:');
                            Log::info($result);
                            
                            if ($result['status'] == 'staged' || $result['status'] == 'processing') {
                                sleep(5);
                                $timeout++;
                            }
        
                        } while (($result['status'] == 'staged' || $result['status'] == 'processing') && $timeout < $max_timeout);
                        
                        if ($timeout >= $max_timeout) {
                            $data['status'] = 'error';
                            $data['message'] = __('Midjourney image generation timed out. Please try again.');
                            return $data;
                        }
                        ...
                        if ($result['status'] == 'finished') {
                            ...
                            foreach ($result['image_urls'] as $url) {
                                ...
                                if (curl_errno($curl)) {
                                    Log::error('Curl error downloading image: ' . curl_error($curl));
                                    curl_close($curl);
                                    continue;
                                }
                                curl_close($curl);
                                ...
                                if (empty($contents)) {
                                    Log::error('Empty image content from URL: ' . $url);
                                    continue;
                                }
            ...
            } elseif ($request->vendor == 'google') {

                $clipdrop = new NanoBanana;
   
                for ($i=0; $i < $max_results; $i++) {
                    $response = $clipdrop->generate($request->prompt, $request->model);
                
                    if ($response['status'] == 'success') {                    

                        $contents = base64_decode($response['result']);

                        $name = 'google-' . Str::random(10) . '.png';
            ...
            if ($image) {
                if ($image->user_id == Auth::user()->id){
                    ...
                    switch ($image->vendor) {
                        ...
                        case 'google': $image_vendor = 'Google'; break;
                    }
                    ...
                    if (is_null($image->vendor_engine)) {
                        ...
                        switch ($image->vendor_engine) {
                            ...
                            case 'gpt-image-1': $image_engine = 'GPT Image'; break;
                            case 'gpt-image-1-mini': $image_engine = 'GPT Image Mini'; break;
                            case 'gpt-image-1.5': $image_engine = 'GPT Image 1.5'; break;
                            ...
                            case 'sd3.5-flash': $image_engine = 'SD 3.5 Flash'; break; 		
                            ...
                            case 'gemini-2.5-flash-image': $image_engine = 'Nano Banana'; break;
                            case 'gemini-3-pro-image-preview': $image_engine = 'Nano Banana Pro'; break;
    ...
    public function checkCredits($model) 
    {
        ...
        switch ($model) {
            ...
            case 'dall-e-3-hd':
                ...
                break;
            case 'gpt-image-1':
                $status = HelperService::checkMediaCredits('gpt_image_1');
                break;
            case 'gpt-image-1-mini':
                $status = HelperService::checkMediaCredits('gpt_image_1_mini');
                break;
            case 'gpt-image-1.5':
                $status = HelperService::checkMediaCredits('gpt_image_1_5');
                break;
            ...
            case 'stable-diffusion-xl-1024-v1-0':
                ...
                break;
            case 'sd3.5-flash':
                $status = HelperService::checkMediaCredits('sd_3_flash');
                break;
            case 'sd3.5-medium':
            ...
            case 'gemini-2.5-flash-image':
                $status = HelperService::checkMediaCredits('google_nano_banana');
            case 'gemini-3-pro-image-preview':
                $status = HelperService::checkMediaCredits('google_nano_banana_pro');
                break;
        }
	...
	*/
    public function updateBalance($images, $model) {
        ...
        switch ($model) {
            ...
            case 'dall-e-3-hd':
                ...
                break;
            case 'gpt-image-1':
                HelperService::updateMediaBalance('gpt_image_1', $images);
                break;
            case 'gpt-image-1-mini':
                HelperService::updateMediaBalance('gpt_image_1_mini', $images);
                break;
            case 'gpt-image-1.5':
                HelperService::updateMediaBalance('gpt_image_1_5', $images);
                break;
            case 'stable-diffusion-v1-6':
            ...
            case 'stable-diffusion-xl-1024-v1-0':
                ...
                break;
            case 'sd3.5-flash':
                HelperService::updateMediaBalance('sd_3_flash', $images);
                break;
            case 'sd3.5-medium':
            ...
            case 'clipdrop':
                ...
                break;
            case 'gemini-2.5-flash-image':
                HelperService::updateMediaBalance('google_nano_banana', $images);
            case 'gemini-3-pro-image-preview':
                HelperService::updateMediaBalance('google_nano_banana_pro', $images);
                break;
        }
    ...
    public function taskCost($model) {
        ...
        switch ($model) {
            ...
            case 'gpt-image-1': return $credits->gpt_image_1; break;
            case 'gpt-image-1-mini': return $credits->gpt_image_1_mini; break;
            case 'gpt-image-1.5': return $credits->gpt_image_1_5; break;
            ...
            case 'clipdrop': return $credits->clipdrop; break;
            case 'gemini-2.5-flash-image': return $credits->google_nano_banana; break;
            case 'gemini-3-pro-image-preview': return $credits->google_nano_banana_pro; break;
            default: return 1;
        }
    }
