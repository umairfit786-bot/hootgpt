...
class PurchaseHistoryController extends Controller
{
    ...
    {   
        ...
        return view('user.purchase.user_purchase_index');
    }
    ...
    {        
                    ...
                    ->addColumn('custom-gateway', function($row){
                        switch ($row['gateway']) {
                            ...
                            case 'TwoCheckout':
                                ...
                                break;
                            case 'Wallet':
                                $custom_gateway = '<div class="overflow-hidden"><img alt="Wallet Gateway" class="w-30" src="' . URL::asset('img/payments/wallet.avif') . '" title="Wallet Balance"></div>';
                                break;
                            case 'Awdpay':
                                $custom_gateway = '<div class="overflow-hidden"><img alt="Awdpay Gateway" class="w-20" src="' . URL::asset('img/payments/awdpay.png') . '"></div>';
                                break;
                            case 'Manual':
        ...
        return view('user.purchase.user_purchase_subscription');
    }
    ...
    public function show(Payment $id)
    {
        if ($id->user_id == Auth::user()->id){
            ...
            return view('user.purchase.user_purchase_show', compact('id'));     
    ...
    public function uploadConfirmation(Request $request) {
        ...
        if (request()->has('confirmation')) {
            ...
            try {
                Mail::to($admin)->send(new BankTransferInvoiceUploaded($payment));
            } catch (Exception $e) {
