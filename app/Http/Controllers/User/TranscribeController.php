	...
	public function process(Request $request) 
    {
        if ($request->ajax()) {
            ...
            if (auth()->user()->minutes != -1) {
                if ((auth()->user()->minutes + auth()->user()->minutes_prepaid) < $audio_length) {
                    if (!is_null(auth()->user()->member_of)) {
                        if (auth()->user()->member_use_credits_speech) {
                            $member = User::where('id', auth()->user()->member_of)->first();
                            if (($member->minutes + $member->minutes_prepaid) < $audio_length) {
                                $data['status'] = 'error';
            ...
            if (isset($response['text'])) {
                ...
                $data['old'] = auth()->user()->minutes + auth()->user()->minutes_prepaid;
                $data['current'] = auth()->user()->minutes + auth()->user()->minutes_prepaid - floor($audio_length);
                return $data; 
	...
	*/
    public function updateBalance($minutes) {
        ...
        if (auth()->user()->minutes != -1) {
            ...
            if (Auth::user()->minutes > $minutes) {

                $total_minutes = Auth::user()->minutes - $minutes;
                $user->minutes = ($total_minutes < 0) ? 0 : $total_minutes;
    
            } elseif (Auth::user()->minutes_prepaid > $minutes) {
    
                $total_minutes_prepaid = Auth::user()->minutes_prepaid - $minutes;
                $user->minutes_prepaid = ($total_minutes_prepaid < 0) ? 0 : $total_minutes_prepaid;
    
            } elseif ((Auth::user()->minutes + Auth::user()->minutes_prepaid) == $minutes) {
    
                $user->minutes = 0;
                $user->minutes_prepaid = 0;
                ...
                if (!is_null(Auth::user()->member_of)) {
                    ...
                    if ($member->minutes > $minutes) {
    
                        $total_minutes = $member->minutes - $minutes;
                        $member->minutes = ($total_minutes < 0) ? 0 : $total_minutes;
            
                    } elseif ($member->minutes_prepaid > $minutes) {
            
                        $total_minutes_prepaid = $member->minutes_prepaid - $minutes;
                        $member->minutes_prepaid = ($total_minutes_prepaid < 0) ? 0 : $total_minutes_prepaid;
            
                    } elseif (($member->minutes + $member->minutes_prepaid) == $minutes) {
            
                        $member->minutes = 0;
                        $member->minutes_prepaid = 0;
            
                    } else {
                        $remaining = $minutes - $member->minutes;
                        $member->minutes = 0;
        
                        $prepaid_left = $member->minutes_prepaid - $remaining;
                        $member->minutes_prepaid = ($prepaid_left < 0) ? 0 : $prepaid_left;
                    }
    
                    $member->update();
    
                } else {
                    $remaining = $minutes - Auth::user()->minutes;
                    $user->minutes = 0;
    
                    $prepaid_left = Auth::user()->minutes_prepaid - $remaining;
                    $user->minutes_prepaid = ($prepaid_left < 0) ? 0 : $prepaid_left;
                }
            }    
        }
