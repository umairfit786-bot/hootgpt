...
class SmartEditorController extends Controller
{
    ...
    {   
        ...
        $favorite_custom_templates = CustomTemplate::select('custom_templates.*', 'favorite_templates.*')->where('favorite_templates.user_id', auth()->user()->id)->where('custom_templates.user_id', auth()->user()->id)->join('favorite_templates', 'favorite_templates.template_code', '=', 'custom_templates.template_code')->where('status', true)->get();  
	...
	public function show(Request $request) 
    {
        ...
        if ($request->ajax()) {
            ...
            if ($request->type == 'original') {
                ...
                $template = CustomTemplate::where('template_code', $request->code)->where('user_id', auth()->user()->id)->first();
    ...
    {        
        ...
        return response()->stream(function () use($content_id, $temperature) {
            ...
            try {
                ...
                if (in_array($model, ['o1', 'o1-mini', 'o3', 'o3-mini', 'o4-mini', 'gpt-5', 'gpt-5-mini', 'gpt-5-nano', 'gpt-5-chat-latest'])) {
