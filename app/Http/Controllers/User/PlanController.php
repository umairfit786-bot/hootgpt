...
class PlanController extends Controller
{
    ...
    public function index()
    {
        if (auth()->user()->hidden_plan) {
            $monthly = SubscriptionPlan::where('payment_frequency', 'monthly')->where(function ($q) {
                $q->where('status', 'active')->orWhere('status', 'hidden'); })->count();
            $yearly = SubscriptionPlan::where('payment_frequency', 'yearly')->where(function ($q) {
                $q->where('status', 'active')->orWhere('status', 'hidden'); })->count();
            $lifetime = SubscriptionPlan::where('payment_frequency', 'lifetime')->where(function ($q) {
                $q->where('status', 'active')->orWhere('status', 'hidden'); })->count();
        } else {
        ...
        if (auth()->user()->hidden_plan) {
            $monthly_subscriptions = SubscriptionPlan::where('payment_frequency', 'monthly')->where(function ($q) {
                $q->where('status', 'active')->orWhere('status', 'hidden'); })->orderBy('price', 'asc')->get();
            $yearly_subscriptions = SubscriptionPlan::where('payment_frequency', 'yearly')->where(function ($q) {
                $q->where('status', 'active')->orWhere('status', 'hidden'); })->orderBy('price', 'asc')->get();
            $lifetime_subscriptions = SubscriptionPlan::where('payment_frequency', 'lifetime')->where(function ($q) {
                $q->where('status', 'active')->orWhere('status', 'hidden'); })->orderBy('price', 'asc')->get();
        } else {
            $monthly_subscriptions = SubscriptionPlan::where('status', 'active')->where('payment_frequency', 'monthly')->orderBy('price', 'asc')->get();
            $yearly_subscriptions = SubscriptionPlan::where('status', 'active')->where('payment_frequency', 'yearly')->orderBy('price', 'asc')->get();
            $lifetime_subscriptions = SubscriptionPlan::where('status', 'active')->where('payment_frequency', 'lifetime')->orderBy('price', 'asc')->get();
        }
        ...
        return view('user.plans.user_plan_index', compact('monthly', 'yearly', 'monthly_subscriptions', 'yearly_subscriptions', 'prepaids', 'prepaid', 'lifetime', 'lifetime_subscriptions', 'plan', 'date', 'time', 'subscriber'));
    }
    ...
    public function subscribe(SubscriptionPlan $id)
    {
        ...
        if ($id->free) {
            ...
            return view('user.plans.user_plan_subscribe-checkout', compact('id', 'payment_platforms', 'tax_value', 'total_value', 'currency', 'gateway_plan_id', 'bank', 'bank_order_id'));
        }
    ...
    public function checkout(Request $request)
    {
        ...
        return view('user.plans.user_plan_prepaid-checkout', compact('id', 'type', 'payment_platforms', 'tax_value', 'total_value', 'currency', 'bank', 'bank_order_id'));
    }
