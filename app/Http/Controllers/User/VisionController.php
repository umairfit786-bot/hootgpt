...
class VisionController extends Controller
{
        ...
        if (auth()->user()->group == 'user') {
            if (config('settings.vision_access_user') != 'allow') {
               ...
               return redirect()->route('user.plans');
                ...
                return view('user.vision.index', compact('chat', 'messages', 'categories', 'prompts', 'brands', 'brands_feature'));
            }
	...
	*/
    ...
    {       
        ...
        $verify = HelperService::creditCheck($this->model, 20);
	...
	*/
        ...
        return response()->stream(function () use($conversation_id) {
            ...
            if (is_null($chat_message->images)) {
                ...
                if (!empty($text)) {
                    ...
                    Log::info($tokens);
                    HelperService::updateBalance($words, $this->model, $tokens['prompt_tokens'], $tokens['completion_tokens']); 
    ...
    private function handleTextChat($openai_key, $chat_conversation, $chat_message, &$text)
    {
        ...
        if (!empty($text)) {
            # Update credit balance
            $words = count(explode(' ', ($text)));
            HelperService::updateBalance($words, $this->model, $input_tokens, $output_tokens);   
