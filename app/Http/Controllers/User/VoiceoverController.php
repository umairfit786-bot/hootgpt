...
class VoiceoverController extends Controller
{
                ...
                if ((auth()->user()->characters + auth()->user()->characters_prepaid) < $total_characters) {
                    if (!is_null(auth()->user()->member_of)) {
                        if (auth()->user()->member_use_credits_voiceover) {
                            ...
                            if (($member->characters + $member->characters_prepaid) < $total_characters) {
                                return response()->json(["error" => __("Not enough available characters to process")], 422);
                            }
                        } else {
                            return response()->json(["error" => __("Not enough available characters to process")], 422);
    ...
    {   
        ...
        if (\App\Services\HelperService::extensionSpeechifyTextToSpeech()) {
            $speechify = new \App\Services\SpeechifyTTSService();
        }

        if (\App\Services\HelperService::extensionLemonfoxTextToSpeech()) {
            $lemonfox = new \App\Services\LemonfoxTTSService();
        }
        ...
        switch($voice->vendor) {
            ...
            case 'lemonfox':
                if (\App\Services\HelperService::extensionLemonfoxTextToSpeech()) 
                    return $lemonfox->synthesizeSpeech($voice, $text, $format, $file_name);
                break;
        }
    }
